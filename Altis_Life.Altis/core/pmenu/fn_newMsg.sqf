...
switch(_type) do
{
	...
	case 1:
	{
		...
		if(_msg == "") exitWith {hint "Zum senden musst du eine Nachricht verfassen!";ctrlShow[88885, true];};
		[[life_smartphoneTarget,_msg,player,0],"TON_fnc_handleMessages",false] spawn life_fnc_MP;
		hint format["Du sendest %1 eine Nachricht: %2",name life_smartphoneTarget,_msg];	
		ctrlShow[88885, true];
		closeDialog 88883;
	};
	//copmessage
	case 2:
	{
		...
		if(_msg == "") exitWith {hint "Zum senden musst du eine Nachricht verfassen!";ctrlShow[888895,true];};
